...
static int bootctrl_load(nxBootctrl_t *ptBootctrl, bool read_only) {
    ...
    ALOGD("%s %s, BOOTCTRL_OFFSET = 0x%" PRIx64, NX_DEBUG, __FUNCTION__, (uint64_t)BOOTCTRL_OFFSET);
...
static int bootctrl_store(int fd, nxBootctrl_t *ptBootctrl) {
    ...
    if (ptBootctrl != NULL) {
        ALOGI("%s %s ptBootctrl = 0x%" PRIx64, NX_DEBUG, __FUNCTION__, (uint64_t)ptBootctrl);
